...
fun Navigation(
    ...
    NavHost(
        navController = rememberNavController(), startDestination = NavigationScreen.UserScreen.name
    ) {
        composable(NavigationScreen.UserScreen.name) {
            UserScreen(
                state = userState.value, onEvent = userViewModel::onEvent, modifier = modifier
            )
        }
        composable(NavigationScreen.FishList.name) {
            FishListScreen(
                state = fishState.value, onEvent = fishViewModel::onEvent, modifier = modifier
            )
        }
